            ...
            $query =
                    "
                        SELECT A.PASIEN_ID, EPISODE_ID, POLI_ID, DOKTER_ID, RESOURCE_ID,
                                ...
                                (SELECT A FROM WEB_CO_DIAGNOSA_DT WHERE SHOW_ITEM='1' AND PASIEN_ID=A.PASIEN_ID AND EPISODE_ID=A.EPISODE_ID AND CREATED_DATE=(SELECT MAX(CREATED_DATE) FROM WEB_CO_DIAGNOSA_DT WHERE SHOW_ITEM='1' AND PASIEN_ID=A.PASIEN_ID AND EPISODE_ID=A.EPISODE_ID))ASSESSMENT,
                                (SELECT TO_CHAR(CREATED_DATE - INTERVAL '7' HOUR,'YYYY-MM-DD')||'T'||TO_CHAR(CREATED_DATE - INTERVAL '7' HOUR,'HH24:MI:SS')||'+00:00' FROM WEB_CO_DIAGNOSA_DT WHERE SHOW_ITEM='1' AND PASIEN_ID=A.PASIEN_ID AND EPISODE_ID=A.EPISODE_ID AND CREATED_DATE=(SELECT MAX(CREATED_DATE) FROM WEB_CO_DIAGNOSA_DT WHERE SHOW_ITEM='1' AND PASIEN_ID=A.PASIEN_ID AND EPISODE_ID=A.EPISODE_ID))CREATEDDATE,
