<?php
    ...
    class Questionnaireresponse extends MY_Satusehat_Controller{

        public function __construct(){
            parent::__construct();
            $this->load->model("Modelquestionnaireresponse", "md");
        }
        
        public function qpoliklinik_post(){
            if(!isset(self::$oauth['issue'])){
                ...
                if(!empty($result)){
                    foreach ($result as $a){
                        ...
                        $questionnaireresourceitem['answer'][]['valueInteger'] = (int)$a->SKORNYERI;
                        $questionnaireresourceitem['linkId']                   = "Skala Nyeri";
                        $questionnaireresourceitem['text']                     = "Skala nyeri";

                        $questionnaireresource['author']['reference']    = "Practitioner/".$practitionerid;
                        $questionnaireresource['authored']               = $a->CREATEDDATE;
                        $questionnaireresource['encounter']['reference'] = "Encounter/".$a->RESOURCE_ID;
                        $questionnaireresource['item'][]                 = $questionnaireresourceitem;
                        $questionnaireresource['questionnaire']          = "Questionnaire/skrining-nyeri";
                        $questionnaireresource['resourceType']           = "QuestionnaireResponse";
                        $questionnaireresource['status']                 = "completed";
                        $questionnaireresource['source']['reference']    = "Patient/".$patientid;
                        $questionnaireresource['source']['display']      = $patientname;
                        $questionnaireresource['subject']['reference']   = "Patient/".$patientid;
                        $questionnaireresource['subject']['display']     = $patientname;

                        $questionnaire['fullUrl']           = "urn:uuid:".Satusehat::uuid();
                        $questionnaire['request']['method'] = "POST";
                        $questionnaire['request']['url']    = "QuestionnaireResponse";
                        $questionnaire['resource']          = $questionnaireresource;

                        $body['resourceType'] = "Bundle";
                        $body['type']         = "transaction";
                        $body['entry'][]      = $questionnaire;

                        // $this->response($questionnaireresource);
                        ...
                        if(isset($response['entry'])){
                            ...
                            if($response === null){
                                $statusColor = "red";
                                echo formatlogbundle($pasienid,$episodeid,'QuestionnaireResponse','','ERROR | response | NULL response from SATUSEHAT',$statusColor);
                            }else{
                                if(isset($response['fault'])){
                                    ...
                                    echo formatlogbundle($pasienid,$episodeid,'QuestionnaireResponse','',$statusMsg,$statusColor);
                                }else{
                                    ...
                                    if(isset($response['issue'])){
                                        if($response['issue'][0]['code']==="duplicate"){
                                            ...
                                            $responsegetServiceRequest = Satusehat::getdata("QuestionnaireResponse","identifier",$identifier,$this->oauth['access_token']);
                                            ...
                                            if(isset($responsegetServiceRequest['entry'])){
                                                foreach($responsegetServiceRequest['entry'] as $responsegetServiceRequests){
                                                    ...
                                                    $statusColor = "yellow";
                                                    $statusMsg   = "GET QuestionnaireResponse BY ACSN";
